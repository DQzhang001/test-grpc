...
function DataCtrl($scope) {
  ...
  $.each($scope.testSuits['suits'],function (i,suit) {
    var startd = new Date(Date.parse(suit.starttime.replace(/-/g, "/"))); 
    var endd = new Date(Date.parse(suit.endtime.replace(/-/g, "/"))); 
    suit['totaltime']=(endd.getTime()-startd.getTime())/1000.0;
